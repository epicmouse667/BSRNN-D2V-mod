...
def FRA_RIR(nsource=1, sr=16000, direct_range=[-6, 50], max_T60=0.8,
    ...
    resample1 = torchaudio.transforms.Resample(sample_sr, sample_sr//int(np.sqrt(ratio)))
    resample2 = torchaudio.transforms.Resample(sample_sr//int(np.sqrt(ratio)), sr)
    ...
    rir_hp = torchaudio.functional.highpass_biquad(rir_downsample, sample_sr//int(np.sqrt(ratio)), 80.)
...
if __name__ == '__main__':
    rir, direct_rir = FRA_RIR(nsource=1)
