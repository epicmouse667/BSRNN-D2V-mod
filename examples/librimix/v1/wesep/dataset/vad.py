...
class VoiceActivityDetection():
    ...
    def segmentation(self, overlap, slice_len):
        # signal, _ = sf.read(self.wave)
        # frequency, signal = wavfile.read(self.wave)
        frequency = 16000
        signal = self.wave
    ...
    def select(self):
        ...
        for audio in audio_slices:
            ...
            chunk_slice = np.arange(0, len(audio)+chunk_len, chunk_len, dtype=np.intc)
        ...
        threshold = np.quantile(energies, .25)
        print(threshold)
        ...
        for audio in audio_slices:
            ...
            if count >= 5:
                ...
                if len(audio) < self.slice_len*16000:
                    ...
                    audio = np.concatenate([audio, np.zeros(self.slice_len*16000 - len(audio))]) 
                fin_audios.append(audio)
                
            i += 1
        # print(audio)
        # print(len(fin_audios))
        if len(fin_audios) == 0:
...
if __name__ == "__main__":
    ...
    wave, _ = sf.read('/workspace2/junjie/datasets/Libri2Mix/wav16k/max/train-100/s1/1578-6379-0038_6415-111615-0009.wav')
    

    spk2_wavs = VoiceActivityDetection(wave)
    # spk1_wavs = VoiceActivityDetection(wave=
    #                                    '/workspace2/junjie/datasets/Libri2Mix/wav16k/max/train-100/s1/1669-129074-0020_2817-142380-0031.wav')
    
    
    spk2_wavs = spk2_wavs.select()
    # spk1_wavs = spk1_wavs.select()

    # print(spk1_wavs)
    
    # spk1_ran = random.randint(0, len(spk1_wavs)-1)
    # spk2_ran = random.randint(0, len(spk2_wavs)-1)

            # print(spk1_wavs)
    # print("spk1: ", len(spk1_wavs[spk1_ran]))
    # print("spk2: ", len(spk2_wavs[spk2_ran]))
