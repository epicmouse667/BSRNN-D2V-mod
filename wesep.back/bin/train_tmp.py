...
def train(config='conf/config.yaml', **kwargs):
    ...
    # scheduler
    scheduler = ReduceLROnPlateau(optimizer, 'min', factor=0.5, verbose=True, patience=5, min_lr=1e-5)

    if rank == 0:
        logger.info("<== Scheduler ==>")
        logger.info("scheduler is: " + "ReduceLROnPlateau")
    # save config.yaml
    if rank == 0:
