...
class ConvTasNet(nn.Module):
    def __init__(self,
        ...
        self.FuseLayer = SpeakerFuseLayer(fuse_type='concat')
    ...
    def forward(self, x, enroll):
        ...
        w = self.encoder(x)
        en = torch.unsqueeze(enroll, 2)
        en_t = self.Transform(en)
        w_en = self.FuseLayer(w, en_t)
        # n x N x L => n x B x L
        e = self.LayerN_S(w_en)
        e = self.BottleN_S(e)
        # n x B x L => n x B x L
        e = self.separation(e)
        # n x B x L => n x N x L
