...
class BSNet(nn.Module):
    def __init__(self, in_channel, spk_emb_dim, nband=7):
        super(BSNet, self).__init__()

        self.nband = nband
        self.feature_dim = in_channel // nband
        self.spk_emb_dim = spk_emb_dim  ## TODO:
        # input_shape = (B,  )
        self.band_rnn = ResRNN(self.feature_dim + self.spk_emb_dim, self.feature_dim * 2)  ## TODO:
        self.band_comm = ResRNN(self.feature_dim + self.spk_emb_dim, self.feature_dim * 2)
    ...
    def forward(self, input):
        ...
        B, N, T = input.shape
        assert N // self.nband - self.feature_dim == self.spk_emb_dim, "Incompatible speaker embedding dim , expected : {}, got :{}".format(
            self.spk_emb_dim, N - self.spk_emb_dim)

        band_output = self.band_rnn(input.view(B * self.nband, self.feature_dim + self.spk_emb_dim, -1)).view(B,
                                                                                                              self.nband,
                                                                                                              -1, T)
...
class BSRNN(nn.Module):
    ...
    def __init__(self, spk_emb_dim=256, sr=16000, win=512, stride=128, feature_dim=128, num_repeat=6):
        ...
        self.conv = nn.Conv1d(self.spk_emb_dim, self.spk_emb_dim // 2, 1)
        self.spk_emb_dim = self.spk_emb_dim // 2
        ...
        for i in range(num_repeat):
            self.separator.append(BSNet(self.nband * self.feature_dim, self.spk_emb_dim, self.nband))
        ...
        for i in range(self.nband):
            self.mask.append(
                nn.Sequential(nn.GroupNorm(1, self.feature_dim + self.spk_emb_dim, torch.finfo(torch.float32).eps),
                              nn.Conv1d(self.feature_dim + self.spk_emb_dim, self.feature_dim * 4, 1),
                              nn.Tanh(),
    ...
    def forward(self, input, enroll):
        # input shape: (B, C, T)
        
        wav_input = input
        spk_emb_input = enroll
        ...
        spk_emb_input = self.conv(spk_emb_input.transpose(0, 1)).transpose(0, 1)
        T = subband_feature.shape[-1]
        spk_emb_repeat = spk_emb_input.unsqueeze(1).unsqueeze(3).repeat(1, self.nband, 1, T)
        print('fuck', subband_feature.size())
        subband_feature = torch.cat([subband_feature, spk_emb_repeat], dim=2)
        print(subband_feature.size())
        iput = (subband_feature.view(batch_size * nch, self.nband * (self.feature_dim + self.spk_emb_dim), -1))
        sep_output = self.separator(iput)
        # B, nband*N, T

        sep_output = sep_output.view(batch_size * nch, self.nband, self.feature_dim + self.spk_emb_dim, -1)
...
if __name__ == '__main__':
    ...
    model = BSRNN(spk_emb_dim=256, sr=16000, win=512, stride=128, feature_dim=128, num_repeat=6)
